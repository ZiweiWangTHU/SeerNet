...
def count_convNd(m: _ConvNd, x: (torch.Tensor,), y: torch.Tensor):
    ...
    # m.total_ops=torch.DoubleTensor([int(10000000000)])
def count_bop_convNd(m: _ConvNd, x: (torch.Tensor,), y: torch.Tensor):
    x = x[0]
    fixed_bit=32
    kernel_ops = torch.zeros(m.weight.size()[2:]).numel()  # Kw x Kh
    ...
    total_ops = y.nelement() * (m.in_channels  * kernel_ops) * fixed_bit * fixed_bit
