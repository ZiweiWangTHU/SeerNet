...
def get_split_train_dataset(dataset_name, batch_size, n_worker, val_size, train_size=None, random_seed=1,
    ...
    if dataset_name == 'imagenet':
        ...
        if dataset_name=='cifar-10':
            ...
            trainset = dataloader(root='./data/cifar/data', train=True, download=False, transform=transform_train)
            train_loader = data.DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=n_worker,pin_memory=True)

            testset = dataloader(root='./data/cifar/data', train=False, download=False, transform=transform_test)
            val_loader = data.DataLoader(testset, batch_size=batch_size, shuffle=False, num_workers=n_worker,pin_memory=True)
            n_class = 10
        else:
