#Adapted from [haq](https://github.com/mit-han-lab/haq/blob/master/lib/utils/quantize_utils.py)
...
# from models.alexnet import Q_AlexNet,QuantAlexNet
...
class QModule(nn.Module):
    ...
    def _quantize_weight(self, weight):
        ...
        if self._quantized and self._w_bit > 0:
            ...
            # print('ori_w:',ori_w.detach())

            # print('quant_w:',w.detach())

            # print('weight quant type',w.type())
            # print('_quantize_weight')
            # STE
            if self._fix_weight:
                # w = w.detach()
                return w.detach()
