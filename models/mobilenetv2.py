...
def conv_1x1_bn(inp, oup, conv_layer=nn.Conv2d, half_wave=True):
    if conv_layer == nn.Conv2d:
        ...
        return nn.Sequential(
            conv_layer(inp, oup, 1, 1, 0, bias=False, half_wave=half_wave,w_bit=8,a_bit=8),
            nn.BatchNorm2d(oup),
            nn.ReLU6(inplace=True)
        )
...
class MobileNetV2(nn.Module):
    def __init__(self, num_classes=1000, input_size=224, width_mult=1., block=InvertedResidual, conv_layer=nn.Conv2d):
        ...
        self.features = [conv_bn(3, input_channel, 2, conv_layer=conv_layer)]  #conv1
        ...
        self.features.append(conv_1x1_bn(input_channel, self.last_channel, conv_layer=conv_layer, half_wave=False)) #conv
        ...
        if conv_layer == nn.Conv2d:
            ...
            self.classifier = QLinear(self.last_channel, num_classes,a_bit=8) #Qlinear1
...
def qmobilenetv2(pretrained=False, num_classes=1000, **kwargs):
    ...
    if pretrained:
        # Load pretrained model.
        # path = '/home/wzw/xh/haq/checkpoints/mobilenetv2/qmobilenetv2_0.6_71.23.pth.tar'
        path='/home/wzw/xh/haq/checkpoints/mobilenetv2-150.pth.tar'
        print('==> load pretrained mobilenetv2 model..')
        assert os.path.isfile(path), 'Error: no checkpoint directory found!'
        ch = torch.load(path)
        ch = {n.replace('module.', ''): v for n, v in ch['state_dict'].items()}
